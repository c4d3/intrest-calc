...
function calculate(){
  ...
  // to the number of montly payments.
  ...
  // we have meaningful results to dispaly
  if(isFinite(monthly)){
    ...
    // Result was Not-a-Number or infinte, which means the input
...
// This storage feature will not work in some browsers (Firfox, e.g.) if you
...
window.onload = function(){
  ...
  // Log an error if none are found. This should never be called.
  if(!inputs) console.log("Inputs not found");
  // Loop through the list of inputs and add calculate as the handler for the change event
  for(var i = 0; i < inputs.length; i++){
  	if(inputs[i].addEventListener){
  		inputs[i].addEventListener("change",calculate,false);
...
function getLenders(amount,apr,years,zipcode){
  ...
  // Fetch the contents of that URL using the XMLHttpPRequest object
  ...
  req.onreadystatechange = function(){
    if(req.readyState == 4 && req.status == 200){
      ...
      // Convert the array lender objects to a string of HTML
      var list ="";
...
function chart(principal,interest,monthly,payments){
  ...
  for(var p = 1; p <= payments; p++){
    ...
    for(var year = 1; year * 12 <= payments; year++){
      var x = paymentToX(year*12); // Compute each tick posistion
